...
# CONTEXT (brief)
Ground evidence using `gh issue view {{issue_number}} --json number,title,body,author,createdAt,updatedAt,state,labels,milestone,reactions,comments,linkedPullRequests`. Use Copilot's standard web and execution tools to review images or attached logs when needed. When an attachment contains diagnostic logs or a ZIP of logs, accept only approved HTTPS GitHub attachment URLs and route it through the repository's bounded attachment/archive analyzer in `.github/scripts/issue-triage/bug-report-analyzer.py`; reject traversal, symlink, encrypted, oversized, or suspiciously compressed entries. Save the analyzer's bounded, redacted output (not the raw archive or an independently extracted copy) under `Generated Files/issueReview/{{issue_number}}/logs/` and cite that output in the analysis. Locate the source code in the current workspace (use `rg`/`git grep` as needed). Link related issues and PRs.
