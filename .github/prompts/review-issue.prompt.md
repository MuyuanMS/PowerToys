...
# CONTEXT (brief)
Ground evidence using `gh issue view {{issue_number}} --json number,title,body,author,createdAt,updatedAt,state,labels,milestone,reactions,comments,linkedPullRequests`. Use Copilot's standard web and execution tools to review images or attached logs when needed. When an attachment contains diagnostic logs or a ZIP of logs, accept only approved HTTPS GitHub attachment URLs and use the repository's bounded attachment/archive validation in `.github/scripts/issue-triage/bug-report-analyzer.py` (or an equivalent safe workflow) before reading or extracting anything; reject traversal, symlink, encrypted, oversized, or suspiciously compressed entries. Extract only validated relevant files into `Generated Files/issueReview/{{issue_number}}/logs/` and cite those files in the analysis. Locate the source code in the current workspace (use `rg`/`git grep` as needed). Link related issues and PRs.
