...
# CONTEXT (brief)
Ground evidence using `gh issue view {{issue_number}} --json number,title,body,author,createdAt,updatedAt,state,labels,milestone,reactions,comments,linkedPullRequests`. Use Copilot's standard web and execution tools to review images or attached logs when needed. Scan the issue body and every `comments[].body` value for supported `https://github.com/user-attachments/files/<id>/PowerToysReport_*.zip` links, and pass each candidate through the repository's bounded attachment/archive analyzer in `.github/scripts/issue-triage/bug-report-analyzer.py`; reject traversal, symlink, encrypted, oversized, suspiciously compressed, or unsupported archive entries and do not extract other ZIP names. Save the analyzer's bounded, redacted output (not the raw archive or an independently extracted copy) under `Generated Files/issueReview/{{issue_number}}/logs/` and cite that output in the analysis. Locate the source code in the current workspace (use `rg`/`git grep` as needed). Link related issues and PRs.
