...
This is the startup project for the `PowerToys Run.` It is a WPF desktop application and follows the `Model-View-ViewModel (MVVM)` design pattern. Plugins play the role of `Model` and provide data to `ViewModel.` It also contains helper classes required by the `PowerLauncher` project, including [`PluginManager`](/src/modules/launcher/PowerLauncher/Plugin/PluginManager.cs) and [`QueryBuilder`](/src/modules/launcher/PowerLauncher/Plugin/QueryBuilder.cs). `PluginManager` provides an interface for managing C# plugins. `QueryBuilder` normalizes the user-typed query and builds a separate [`Query`](/src/modules/launcher/Wox.Plugin/Query.cs) object for each enabled plugin whose action keyword matches, retaining only the longest matching keyword when overlapping prefixes exist. When no keyword matches, the query is routed to the enabled global plugins. Each [`Query`](/src/modules/launcher/Wox.Plugin/Query.cs) object contains the action keyword and cleaned query text.
