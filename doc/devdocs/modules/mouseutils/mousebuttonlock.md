...
The unit tests cover the Win32-free `Engine`. The Win32 glue in `dllmain.cpp` (hook install, the message pump, deferred `WM_MBL_INJECT` injection, suppression of the up by returning `1`, and the injection-tag self-filter) was verified live against the running runner with temporary `MBL`-tagged trace logging in `dllmain.cpp` (since removed):
