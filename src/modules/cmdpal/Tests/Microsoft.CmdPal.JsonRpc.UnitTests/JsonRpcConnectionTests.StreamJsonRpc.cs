...
public partial class JsonRpcConnectionTests
{
    ...
    [TestMethod]
    public async Task PendingRequest_FailsPromptly_WhenInboundContentLengthExceedsLimit()
    {
        using var cts = new CancellationTokenSource(TestTimeout);
        var harness = CreateHarness(TimeSpan.FromSeconds(30));
        ...
        try
        {
            ...
            var oversizedHeader = Encoding.ASCII.GetBytes("Content-Length: 16777217\r\n\r\n");
            await harness.ExtensionWrites.WriteAsync(oversizedHeader, cts.Token);
