...
public class SvgIconProtocolTests
{
    ...
    [DataRow(false)]
    [DataRow(true)]
    public void ThemedSvgFileHonorsUtf16EncodingWithoutByteOrderMarkAfterLongWhitespace(bool bigEndian)
    {
        var path = Path.Combine(Path.GetTempPath(), $"CmdPal-{Guid.NewGuid():N}.svg");
        try
        {
            var encoding = new UnicodeEncoding(bigEndian, byteOrderMark: false);
            File.WriteAllBytes(path, encoding.GetBytes(new string(' ', 600) + Template));
    ...
    [DataRow(false)]
    [DataRow(true)]
    public void ThemedSvgFileHonorsUtf32EncodingWithoutByteOrderMarkAfterLongWhitespace(bool bigEndian)
    {
        var path = Path.Combine(Path.GetTempPath(), $"CmdPal-{Guid.NewGuid():N}.svg");
        try
        {
