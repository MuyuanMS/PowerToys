...
namespace Peek.FilePreviewer.Previewers.SqlitePreviewer
{
    public partial class SqlitePreviewer : ObservableObject, ISqlitePreviewer
    {
        ...
        public async Task LoadPreviewAsync(CancellationToken cancellationToken)
        {
            ...
            TableCountText = ResourceLoaderInstance.FormatString("Sqlite_Table_Count", tableNames.Count);
                ...
                while (await reader.ReadAsync(cancellationToken))
                {
                    ...
                    foreach (var col in tableInfo.Columns)
                    {
                        ...
                        else if (reader.GetFieldType(i) == typeof(byte[]))
                        {
                            ...
                            row[col.BindingKey] = ResourceLoaderInstance.FormatString("Sqlite_Blob_Value", blobStream.Length);
                        }
                        else
                        {
